...
import { selectSeats, findBookings } from '../../actions/bookingActions';

class PlaceSelection extends React.Component 
{
    constructor(props) {
        super(props);
        this.props.findBookings(this.props.selectedMovie._id, this.props.selectedShowing);
    ...
    render() {
        if (!this.props.selectedMovie || !this.props.selectedShowing) {  
            return (
                ...
                <NavLink className = "arrowleft" to = "/booking/tickets" > <i className = "fas fa-angle-double-left"> </i></NavLink>
                    <div className="option">
                        <div className="empty" style={{ backgroundColor: "rgba(0,0,0,0)" }}>
                            You have to choose movie and showing time first
                        </div>
                    </div>
                    <NavLink className = "arrowright" to = "/booking/personal-details"> <i className = "fas fa-angle-double-right"> </i></NavLink>
                </div>
            );
        }
        return (
